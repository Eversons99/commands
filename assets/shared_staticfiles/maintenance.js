...
async function searchOnts(operationMode) {
    setIdentificator()
    loadingAnimation(true)
    const baseUrl = 'http://10.0.50.204:8000/maintenance/shared_core'
    const sourceHost = document.getElementById('select-olt').value
    ...
    if (operationMode == 'attenuator' && responseofOntsRequest.error == true) {
        ...
        return window.location = `http://10.0.50.204:8000/maintenance/generator/search_onts_via_ssh?tab_id=${tabId}`
    }
...
async function checkFileNameExists(fileName){
    let allFileNames = await fetch(`http://10.0.50.204:8000/maintenance/shared-core/check_file_names?fileName=${fileName}`)
...
async function getMaintenanceInfo(operationMode, registerId) {
    const url = `http://10.0.50.204:8000/maintenance/shared_core/get_maintenance_info`
    const requestOptions = {
        method: 'POST',
        headers: {
            'Content-type': 'application/json',
...
async function downloadCommandsFile(operationMode, registerId) {
    const url = `http://10.0.50.204:8000/maintenance/shared_core/download_command_file?tab_id=${registerId}&mode=${operationMode}`
...
async function discardCommands(operationMode, registerId) {
    ...
    const url = `http://10.0.50.204:8000/maintenance/shared_core/discard_commands`
    const requestOptions = {
        method: 'DELETE',
        headers: {
            'Content-type': 'application/json',
    ...
    if(currentUrl.includes('/render_page_commands')){
        ...
        return window.location = 'http://10.0.50.204:8000/maintenance/'
    } else {
        alert(removeCommands.message)
...
async function updateStatusAppliedCommands(operationMode, maintenanceInfo, rollback){
    ...
    const url = `http://10.0.50.204:8000/maintenance/shared_core/update_status_applied_commands?${queryParams}`
