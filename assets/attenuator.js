async function saveInitialAttenuationState() {
    ...
    const baseUrl = "10.0.30.157:8000/attenuator"
...
async function showAttenuation(attenuationId) {
    ...
    let allAttenuations = await fetch('10.0.30.157:8000/attenuator/get_onts_to_render', requestOptions)
...
async function discardAttenuation(attenuationId) {
    ...
    let discardSingleAttenuation = await fetch('10.0.30.157:8000/attenuator/discard_attenuation', requestOptions)
    ...
    return window.location = `10.0.30.157:8000/attenuator/render_attenuations_page?tab_id=${getIdentificator()}`
}

function maintainAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `10.0.30.157:8000/attenuator/render_attenuations_page?tab_id=${tabId}`
}

function nextAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `10.0.30.157:8000/attenuator/next_attenuation?tab_id=${tabId}`
}
...
async function endAttenuation() {
    ...
    const tabId = getIdentificator()
    let endAttenuations = await fetch(`10.0.30.157:8000/attenuator/end_attenuations?tab_id=${tabId}`)
    endAttenuations = await endAttenuations.json()

    if (endAttenuations.error) {
        const messageError = endAttenuations.message
        return window.location = `10.0.30.157:8000/attenuator/render_error_page?message=${messageError}`
    }
    return window.location = `10.0.30.157:8000/attenuator/render_page_commands?tab_id=${tabId}`
}
