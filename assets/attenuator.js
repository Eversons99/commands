async function saveInitialAttenuationState() {
    ...
    const baseUrl = 'http://168.0.96.11:50074/maintenance/attenuator'
    ...
    if (saveAttenuationState.error) {
        ...
        return window.location = `http://168.0.96.11:50074/maintenance/shared_core/render_error_page?message=${messageError}`
    }
...
async function showAttenuation(attenuationId) {
    ...
    let allAttenuations = await fetch('http://168.0.96.11:50074/maintenance/attenuator/get_onts_to_render', requestOptions)
...
async function discardAttenuation(attenuationId) {
    ...
    let discardSingleAttenuation = await fetch('http://168.0.96.11:50074/maintenance/attenuator/discard_attenuation', requestOptions)
    ...
    return window.location = `http://168.0.96.11:50074/maintenance/attenuator/render_attenuations_page?tab_id=${getIdentificator()}`
}

function maintainAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `http://168.0.96.11:50074/maintenance/attenuator/render_attenuations_page?tab_id=${tabId}`
}

function nextAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `http://168.0.96.11:50074/maintenance/attenuator/next_attenuation?tab_id=${tabId}`
}
