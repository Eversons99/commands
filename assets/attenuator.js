async function saveInitialAttenuationState() {
    ...
    const baseUrl = 'https://commands.nmultifibra.com.br/maintenance/attenuator'
    ...
    if (saveAttenuationState.error) {
        ...
        return window.location = `https://commands.nmultifibra.com.br/maintenance/shared_core/render_error_page?message=${messageError}`
    }
...
async function showAttenuation(attenuationId) {
    ...
    let allAttenuations = await fetch('https://commands.nmultifibra.com.br/maintenance/attenuator/get_onts_to_render', requestOptions)
...
async function discardAttenuation(attenuationId) {
    ...
    let discardSingleAttenuation = await fetch('https://commands.nmultifibra.com.br/maintenance/attenuator/discard_attenuation', requestOptions)
    ...
    return window.location = `https://commands.nmultifibra.com.br/maintenance/attenuator/render_attenuations_page?tab_id=${getIdentificator()}`
}

function maintainAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `https://commands.nmultifibra.com.br/maintenance/attenuator/render_attenuations_page?tab_id=${tabId}`
}

function nextAttenuation() {
    loadingAnimation(true)
    const tabId = getIdentificator()
    return window.location = `https://commands.nmultifibra.com.br/maintenance/attenuator/next_attenuation?tab_id=${tabId}`
}
...
async function endAttenuation() {
    ...
    const baseUrl = "https://commands.nmultifibra.com.br/maintenance"
    const attenuationsTable = document.getElementById('attenuations-table')
