async function getReadyCommandFiles(){
    ...
    loadingAnimation(false)
    window.location = `http://10.0.50.204:8000/maintenance/files/get_files?filter=${selectedFilter}`
}

async function displayAllLogs(operationMode, registerId){
    const lastFilter = window.location.href
    const url = `http://10.0.50.204:8000/maintenance/files/show_logs?tabId=${registerId}&operationMode=${operationMode}&lastFilter=${lastFilter}`
