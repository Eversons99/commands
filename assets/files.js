async function getReadyCommandFiles(){
    ...
    loadingAnimation(false)
    window.location = `https://commands.nmultifibra.com.br/maintenance/files/get_files?filter=${selectedFilter}`
}

async function displayAllLogs(operationMode, registerId){
    const lastFilter = window.location.href
    const url = `https://commands.nmultifibra.com.br/maintenance/files/show_logs?tabId=${registerId}&operationMode=${operationMode}&lastFilter=${lastFilter}`
