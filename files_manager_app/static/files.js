async function getReadyCommandFiles(){
    ...
    loadingAnimation(false)
    window.location = `http://localhost:8000/maintenance/files/get_files?filter=${selectedFilter}`
}

async function displayAllLogs(operationMode, registerId){
    const lastFilter = window.location.href
    const url = `http://localhost:8000/maintenance/files/show_logs?tabId=${registerId}&operationMode=${operationMode}&lastFilter=${lastFilter}`
